...
class MMVAEWithClassifier(MMVAE):
    def __init__(self, prior_dist, params, *vaes, num_classes=10):
        super(MMVAEWithClassifier, self).__init__(prior_dist, params, *vaes)
        self.classifier = nn.Linear(self.params['latent_dim'], num_classes)  

    def forward(self, x, K=1):
       
        qz_xs, px_zs, zss = super(MMVAEWithClassifier, self).forward(x, K=K)
        
        latent_vectors = torch.stack([zs.mean(0) for zs in zss]).mean(0)  
        class_logits = self.classifier(latent_vectors)  
        
        return qz_xs, px_zs, zss, class_logits

    def predict(self, x, K=1):
        self.eval()
        with torch.no_grad():
            _, _, zss, class_logits = self.forward(x, K=K)
