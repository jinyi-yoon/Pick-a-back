...
class SparsePruner(object):
    ...
    def __init__(self, mode, dataset_name, network_width_multiplier, 
                 finetune_again, target_sparsity, initial_sparsity, pruning_frequency, weight_decay,
                 model, masks, begin_prune_step, end_prune_step, inference_dataset_idx):
        self.finetune_again = finetune_again
        self.mode = mode
        ...
        finetune_again = False if finetune_again == False else finetune_again
        if mode == 'prune' or mode == 'inference' or (mode == 'finetune' and finetune_again):
            self.current_dataset_idx = self.model.datasets.index(dataset_name) + 1 ##### Jinee ##### self.model.module.datasets.index(dataset_name) + 1
        elif mode == 'finetune':
            self.current_dataset_idx = len(self.model.datasets) - 1 ##### Jinee ##### len(self.model.module.datasets) - 1
        else:
            print('We do not support \'{}\' mode'.format(mode))
            sys.exit(-1)
    ...
    def _time_to_update_masks(self, curr_prune_step):
        ...
        is_pruning_step = (
            self.last_prune_step + self.pruning_frequency) <= curr_prune_step
    ...
    def calculate_curr_task_ratio(self):
        ...
        return float(curr_task_elem.cpu()) / total_elem * (self.network_width_multiplier ** 2)
    ...
    def calculate_zero_ratio(self):
        ...
        return float(zero_elem.cpu()) / total_elem * (self.network_width_multiplier ** 2)
    ...
    def apply_mask(self):
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, nl.SharableConv2d) or isinstance(module, nl.SharableLinear):
                ...
                weight[mask.eq(0)] = 0.0 # Jinee) released weight
                weight[mask.gt(self.inference_dataset_idx)] = 0.0 # Jinee) expanded weight
        return
