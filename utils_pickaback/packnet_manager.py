...
class Manager(object):
    ...
    def __init__(self, dataset, checkpoint_format, weight_decay, cuda,
                 model, shared_layer_info, masks, train_loader, val_loader):
        # print('\n\n\n...... [packnet manager] init')
        ...
        self.inference_dataset_idx = self.model.datasets.index(dataset) + 1 ##### Jinee ##### self.inference_dataset_idx = self.model.module.datasets.index(dataset) + 1
        self.pruner = SparsePruner(weight_decay, cuda,
                                   self.model, masks, None, None, self.inference_dataset_idx)
        self.train_loader = train_loader
        self.val_loader   = val_loader

        if dataset == 'face_verification':
    ...
    def train(self, optimizers, epoch_idx, curr_lrs):
        # print('...... [packnet manager] train ...... starts')
        ...
        with tqdm(total=len(self.train_loader), desc='Train Epoch #{}: '.format(epoch_idx + 1), disable=False, ascii=True) as t:
            # print('\n...... [packnet manager] train ...... tqdm starts')
            
            for batch_idx, (data, target) in enumerate(self.train_loader):
                # print('...... [packnet manager] train ...... batch_idx=%d'%batch_idx)
                #if self.cuda:
        ...
        with tqdm(total=len(self.val_loader),
            ...
            with torch.no_grad():
                ...
                for data, target in self.val_loader:
                    # print('... model.features[0].weight[0][0][0] = ', end='')
                    # print(self.model.features[0].weight[0][0][0])
                    # print('... data[0][0][0][0] = ', end='')
                    # print(data[0][0][0][0])
                    # print('... target[0] = ', end='')
                    # print(target[0])
                    print()
        ...
        for name, module in self.model.named_modules(): ##### Jinee ##### self.model.module.named_modules():
            if isinstance(module, nn.Conv2d):
                if module.bias is not None:
                    self.shared_layer_info[self.dataset][
                ...
                self.shared_layer_info[self.dataset]['fc_bias'][name] = module.bias
    ...
    def load_checkpoint(self, optimizers, resume_from_epoch, save_folder):
        # print('...... [packnet manager] load checkpoint')
    ...
    def load_checkpoint_for_inference(self, resume_from_epoch, save_folder):
        # print('...... [packnet manager] load checkpoint for inference')
        ...
        if resume_from_epoch > 0:                    
            ...
            for name, module in self.model.named_modules(): ##### Jinee ##### self.model.module.named_modules():
                if isinstance(module, nn.Conv2d):
                    if module.bias is not None:
                        module.bias = self.shared_layer_info[self.dataset]['conv_bias'][name]
                elif isinstance(module, nn.BatchNorm2d):
                    module.running_mean = self.shared_layer_info[self.dataset]['bn_layer_running_mean'][name]
                    module.running_var = self.shared_layer_info[self.dataset]['bn_layer_running_var'][name]
                    module.weight = self.shared_layer_info[self.dataset]['bn_layer_weight'][name]
                    module.bias = self.shared_layer_info[self.dataset]['bn_layer_bias'][name]
                elif isinstance(module, nn.Linear) and 'features' in name:
                    module.bias = self.shared_layer_info[self.dataset]['fc_bias'][name]
                elif isinstance(module, nn.PReLU):
                    module.weight = self.shared_layer_info[self.dataset]['prelu_layer_weight'][name]
